...
class Produk extends CI_Controller
{
    ...
    function judwul($id)
    {
        $d['content'] = "penjual/jadwal_master";
        $d['tabel'] = $this->toko_online_model->jadwal($id)->result();
        $d['id'] = $id;
    ...
    public function ajax_tabel()
    {
        ...
        if (!$this->input->is_ajax_request()) {
            ...
            $columns = array(
                ...
                array('db' => 'berat_bersih', 'dt' => 'berat'),
    ...
    function edit_jadwal($id)
    {
        if ($this->session->userdata("logged_in") == TRUE) {
            $this->uri->segment(5);
            $d['pre'] = $this->db->where('id_jadwal', $id)->get('jadwal')->row();
            $d['content'] = "penjual/edit_jadwal";
            $d['id'] = $id;
            $this->load->view('penjual/dashboard', $d);
        } else {
            $desk = base_url("");
    ...
    function action_add_jadwal()
    {
        ...
        $this->toko_online_model->add_jadwal($data);

        redirect('penjual/produk/judwul/' . $this->input->post('id_produk'));
    }
    function action_update_jadwal()
    {
        $data = array(
            'tgl_berangkat' => $this->input->post('tgl_berangkat'),
            'tgl_pulang' => $this->input->post('tgl_pulang'),
        ...
        $this->db->where(array('id_jadwal' => $this->input->post('id_jadwal')));
        $this->db->update('jadwal', $data);

        redirect('penjual/produk/judwul/' . $this->input->post('id_produk'));
    }
    function action_delete_jadwal()
    {
        $this->db->where(array('id_jadwal' => $this->input->post('id_jadwal')));
